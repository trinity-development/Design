...
;;;		    "146" "147" "148" "149" "150" "151" "152" "153" "154" "155" "156" "157" "158" "159" "160"));LIST OF ROWS TO TAKE IN EXCEL
  ...
  (while (and
           (< n (vlax-safearray-get-u-bound Orientationarr 1))
           (/= alphaElement nil)
         )
    (setq
    ...
    (while;do while conditions are met
              ...
              (setq currentOrientationCheck (vlax-safearray-get-element Orientationarr n))
              (setq currentOrientationNum (vlax-safearray-get-element OrientationNumarr n))
              ;(alert currentOrientationCheck)
              ;(alert currentOrientationNum)

      (if
        (and
          (/= (vl-catch-all-apply 'vlax-ldata-get (list "currentRoofOrientation" currentOrientationCheck)) currentOrientationNum)
          (/= (vl-catch-all-apply 'vlax-ldata-get (list "currentRoofOrientation" currentOrientationCheck)) nil)
          (= (vl-catch-all-apply 'numberp (list (atoi currentOrientationNum))) T)
        )
        (progn;execute
